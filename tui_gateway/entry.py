...
# bmb modules.  bmb_cli sets BMB_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("BMB_PYTHON_SRC_ROOT", "")
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `BMB_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("BMB_TUI_SIDECAR_URL")
...
# tested; override via ``BMB_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("BMB_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``BMB_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
