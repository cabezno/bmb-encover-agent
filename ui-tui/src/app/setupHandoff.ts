import type { RunExternalProcess } from '@bmb/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`bmb ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching bmb: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`bmb ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
