...
// Mirrors @bmb/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @bmb/ink is bundled separately from packages/.
...
const BRAND: ThemeBrand = {
  name: 'BLACKMAGICBOX AGENT',
  icon: '⚕',
  prompt: '❯',
  welcome: '[CMD] TYPE YOUR MESSAGE OR /HELP FOR COMMANDS.',
...
export const DARK_THEME: Theme = {
  color: {
    ...
    completionBg: '#141414',
    completionCurrentBg: mix('#141414', '#00E5FF', 0.25),
...
// Terminal. Explicit BMB_TUI_THEME / COLORFGBG signals above still win,
...
// `BMB_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `BMB_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `BMB_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `BMB_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default BMB palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.BMB_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.BMB_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.BMB_TUI_BACKGROUND ?? '')
