...
React + Ink terminal UI for BMB. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `BMB_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.bmb/.bmb_history` or under `BMB_ENCOVER_HOME`.
...
```text
ui-tui/
  packages/bmb-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      bmb-ink.d.ts    type declarations for @bmb/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent EncoverCLI subprocess for slash commands
```
