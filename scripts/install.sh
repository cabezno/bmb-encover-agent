#!/bin/bash
# ============================================================================
# BlackMagicBox Encover Agent Installer
# ============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --hermes-home)
            ...
            ;;
        -h|--help)
            echo "BlackMagicBox Encover Agent Installer"
            echo ""
...
print_banner() {
    ...
    echo "│             ⚕ BlackMagicBox Encover Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Encover.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `bmb` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $BMB_ENCOVER_HOME/bmb-encover
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $BMB_ENCOVER_HOME/bmb-encover
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/bmb-encover
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in BMB_ENCOVER_HOME.
    if is_termux; then
        INSTALL_DIR="$BMB_ENCOVER_HOME/bmb-encover"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$BMB_ENCOVER_HOME/bmb-encover/.git" ]; then
            INSTALL_DIR="$BMB_ENCOVER_HOME/bmb-encover"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/bmb-encover)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/bmb-encover"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/Encover/bmb-encover/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$BMB_ENCOVER_HOME/node/bin/node" ]; then
        export PATH="$BMB_ENCOVER_HOME/node/bin:$PATH"
        local found_ver=$("$BMB_ENCOVER_HOME/node/bin/node" --version)
...
install_node() {
    ...
    log_info "Extracting to ~/.bmb/node/..."
    ...
    # Place into ~/.bmb/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$BMB_ENCOVER_HOME/node"
    mkdir -p "$BMB_ENCOVER_HOME"
    mv "$extracted_dir" "$BMB_ENCOVER_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$BMB_ENCOVER_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$BMB_ENCOVER_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$BMB_ENCOVER_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$BMB_ENCOVER_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$BMB_ENCOVER_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.bmb/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "BlackMagicBox Encover Agent itself does not require or retain root access."
...
setup_path() {
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        log_info "bmb not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# BlackMagicBox Encover Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# BlackMagicBox Encover Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# BlackMagicBox Encover Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so bmb works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.bmb directory structure (config at top level, code in subdir)
    mkdir -p "$BMB_ENCOVER_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.bmb/.env (top level, easy to find)
    if [ ! -f "$BMB_ENCOVER_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$BMB_ENCOVER_HOME/.env"
            log_success "Created ~/.bmb/.env from template"
        else
            touch "$BMB_ENCOVER_HOME/.env"
            log_success "Created ~/.bmb/.env"
        fi
    else
        log_info "~/.bmb/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.bmb/config.yaml (top level, easy to find)
    if [ ! -f "$BMB_ENCOVER_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$BMB_ENCOVER_HOME/config.yaml"
            log_success "Created ~/.bmb/config.yaml from template"
        fi
    else
        log_info "~/.bmb/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$BMB_ENCOVER_HOME/SOUL.md" ]; then
        cat > "$BMB_ENCOVER_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.bmb/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.bmb/"

    # Seed bundled skills into ~/.bmb/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.bmb/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.bmb/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$BMB_ENCOVER_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$BMB_ENCOVER_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.bmb/skills/"
        fi
    fi
}
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'bmb setup' after install."
        return 0
    fi
    ...
    # Run bmb setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m bmb_cli.main setup < /dev/tty
    else
        python -m bmb_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$BMB_ENCOVER_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    WHATSAPP_SESSION="$BMB_ENCOVER_HOME/whatsapp/session/creds.json"
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'bmb gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                log_warn "Systemd install failed. You can start manually: bmb gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $BMB_ENCOVER_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $BMB_ENCOVER_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $BMB_ENCOVER_HOME/cron/, sessions/, logs/"
    ...
    echo -e "   ${GREEN}bmb setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}bmb config${NC}       View/edit configuration"
    echo -e "   ${GREEN}bmb config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}bmb gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}bmb update${NC}       Update to latest version"
    echo ""
