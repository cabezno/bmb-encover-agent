#!/usr/bin/env bash
# Canonical test runner for bmb-encover. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.bmb/bmb-encover/venv"; do
...
# Unset BMB_* behavioral vars too.
unset BMB_YOLO_MODE BMB_INTERACTIVE BMB_QUIET BMB_TOOL_PROGRESS \
      BMB_TOOL_PROGRESS_MODE BMB_MAX_ITERATIONS BMB_SESSION_PLATFORM \
      BMB_SESSION_CHAT_ID BMB_SESSION_CHAT_NAME BMB_SESSION_THREAD_ID \
      BMB_SESSION_SOURCE BMB_SESSION_KEY BMB_GATEWAY_SESSION \
      BMB_PLATFORM BMB_INFERENCE_PROVIDER BMB_MANAGED BMB_DEV \
      BMB_CONTAINER BMB_EPHEMERAL_SYSTEM_PROMPT BMB_TIMEZONE \
      BMB_REDACT_SECRETS BMB_BACKGROUND_NOTIFICATIONS BMB_EXEC_ASK \
      BMB_ENCOVER_HOME_MODE 2>/dev/null || true
