"""ACP tool-call helpers for mapping bmb tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map bmb tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a bmb tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some BMB tools append a human hint after a JSON payload, e.g.
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # BMB read_file output is line-numbered with `|`. If we send it as raw
...
def build_tool_start(
    ...
    """Create a ToolCallStart event for the given bmb tool invocation."""
