"""CLI entry point for the bmb-encover ACP adapter.

Loads environment variables from ``~/.bmb/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from BMB_ENCOVER_HOME (default ``~/.bmb``)."""
    from bmb_cli.env_loader import load_bmb_dotenv

    bmb_home = get_bmb_home()
    loaded = load_bmb_dotenv(bmb_home=bmb_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", bmb_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting bmb-encover ACP adapter")
