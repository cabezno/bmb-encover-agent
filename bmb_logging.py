"""Centralized logging setup for BlackMagicBox Encover Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.bmb/logs/`` (profile-aware via ``get_bmb_home()``).
...
from bmb_constants import get_config_path, get_bmb_home
...
# Used by _ComponentFilter and exposed for ``bmb logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the bmb group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from bmb_cli.config import is_managed
