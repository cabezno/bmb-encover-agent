...
from bmb_constants import OPENROUTER_BASE_URL, get_bmb_home
...
if tinker_atropos_dir.exists():
    ...
    os.environ['BMB_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to bmb-encover directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.bmb/config.yaml if not provided)
    ...
    """
    # Load config from ~/.bmb/config.yaml
    config = load_bmb_config()
