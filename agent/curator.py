...
def _state_file() -> Path:
    return get_bmb_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.bmb/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``bmb update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``bmb curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `bmb curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as BMB' background skill CURATOR. This is an "
    ...
    "into ~/.bmb/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: bmb-config-*, bmb-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.bmb/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.bmb/logs/curator/``)
    ...
    authored skill data in ``~/.bmb/skills/``.

    ``ensure_bmb_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_bmb_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every bmb install already has it
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.bmb/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.bmb/skills/.archive/` for "
            "safety and can be restored via `bmb curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.bmb/skills/.archive/`. "
            "Restore any via `bmb curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `bmb curator restore <name>`")
    lines.append("- All archives live under `~/.bmb/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `bmb curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `bmb curator status` can point at it.
        try:
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``bmb model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `bmb model` → auxiliary
