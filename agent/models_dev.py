...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.bmb/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: BMB ↔ models.dev
# ---------------------------------------------------------------------------

# BMB provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → BMB (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_bmb_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a BMB provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from bmb_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal BMB agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a BMB provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve BMB ID → models.dev ID
...
def get_model_info(
    ...
    Accepts BMB or models.dev provider ID.  Tries exact match then
