...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time BMB starts and finds ``~/.openclaw/``.

    Points users at ``bmb claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``bmb claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to BMB, run "
        "`bmb claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `bmb claw cleanup` (renames it to ~/.openclaw.pre-migration — "
