...
as ``kind: backend``) or ``~/.bmb/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``bmb tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``bmb tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``bmb tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$BMB_ENCOVER_HOME/cache/images/``, creating parents as needed."""
    from bmb_constants import get_bmb_home

    path = get_bmb_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$BMB_ENCOVER_HOME/cache/images/``.
