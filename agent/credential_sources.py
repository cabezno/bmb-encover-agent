"""Unified removal contract for every credential source BMB reads from.

BMB seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.bmb/.env
    claude_code   — ~/.claude/.credentials.json
    bmb_pkce   — ~/.bmb/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``bmb auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), bmb_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.bmb/.env  → clear it
    ...
    """
    from bmb_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to BMB.",
            f"  The pool entry is now suppressed — BMB will ignore "
            f"{env_var} until you run `bmb auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so BMB stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `bmb auth add anthropic` to re-enable if needed.",
    ])


def _remove_bmb_pkce(provider: str, removed) -> RemovalResult:
    """~/.bmb/.anthropic_oauth.json is ours — delete it outright."""
    from bmb_constants import get_bmb_home

    result = RemovalResult()
    oauth_file = get_bmb_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared BMB Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `bmb login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `bmb auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the BMB auth store is not enough — _seed_from_singletons() would
    ...
    via ``bmb auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from bmb_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `bmb auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `bmb auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    BMB stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `bmb auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="bmb_pkce",
        remove_fn=_remove_bmb_pkce,
        description="~/.bmb/.anthropic_oauth.json",
    ))
    register(RemovalStep(
