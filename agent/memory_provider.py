...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - bmb_home (str): The active BMB_ENCOVER_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.bmb``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "bmb").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'bmb memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], bmb_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'bmb memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``bmb_home`` is the active BMB_ENCOVER_HOME directory path.
