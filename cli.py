#!/usr/bin/env python3
"""
BlackMagicBox Encover Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the BlackMagicBox Encover Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills bmb-encover-dev,github-auth
...
from bmb_cli.banner import _format_context_length, format_banner_version_label
...
from bmb_cli.browser_connect import (
...
load_bmb_dotenv(bmb_home=_bmb_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.bmb/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``bmb chat --ignore-user-config``),
    the user config at ``~/.bmb/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({BMB_ENCOVER_HOME}/config.yaml)
    user_config_path = _bmb_home / 'config.yaml'
    ...
    if config_path.exists():
        try:
            ...
            # location is model.provider (written by `bmb model`).
    ...
    # Normalize config key: the new config system (bmb_cli/config.py) and all
...
# Initialize centralized logging early — agent.log + errors.log in ~/.bmb/logs/.
...
try:
    from bmb_cli.skin_engine import init_skin_from_config
...
from bmb_cli.banner import build_welcome_banner
from bmb_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from bmb_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run bmb -w")
        return None
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`bmb_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from bmb_constants import get_bmb_home as _get_bmb_home
        _bmb_home_maint = _get_bmb_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_bmb_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`bmb_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``bmb/*`` and ``pr-*`` local branches that
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``bmb/hermes-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``bmb -w`` sessions and PR review
...
from bmb_constants import is_termux as _is_termux_environment
...
def _build_compact_banner() -> str:
    ...
    border_color = _skin.get_color("banner_border", "#00FF88") if _skin else "#00FF88"
    title_color = _skin.get_color("banner_title", "#00FF88") if _skin else "#00FF88"
    dim_color = _skin.get_color("banner_dim", "#009944") if _skin else "#009944"

    if skin_name == "default":
        line1 = "BMB Undercover Agent - AI Agent Framework"
        tiny_line = "BMB UNDERCOVER"
    else:
        agent_name = _skin.get_branding("agent_name", "BMB Undercover Agent") if _skin else "BMB Undercover Agent"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Encover[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from bmb_cli.plugins import get_plugin_manager
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.bmb/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.bmb/config.yaml on first use)
...
class EncoverCLI:
    """
    Interactive CLI for the BlackMagicBox Encover Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from bmb_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `bmb chat --ignore-rules` in bmb_cli/main.py. When true we
        ...
        # it's shared across all Hermes processes for this BMB_ENCOVER_HOME.
        ...
        self._history_file = _bmb_home / ".hermes_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from bmb_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from bmb_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from bmb_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from bmb_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: bmb setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: bmb setup")
            return False
        ...
        # `bmb chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `bmb auth add openai-codex`
        # without `bmb model`), fall back to the provider's first catalog
    ...
    def show_banner(self):
        ...
        if use_compact:
            ...
            # Show 3D rotating cube splash
            try:
                from bmb_cli.splash_cube import show_splash
                show_splash(duration=1.8, fps=18)
            except Exception:
                pass
        ...
        from bmb_cli.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Encover Hermes 3 & 4 models are NOT agentic and are not "
                "designed for use with BlackMagicBox Encover Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.bmb/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from bmb_cli.clipboard import save_clipboard_image

        img_dir = get_bmb_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: bmb --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        if subcmd in ("list", "ls"):
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_bmb_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from bmb_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run bmb chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'bmb setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from bmb_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from bmb_cli.skin_engine import get_active_help_header
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in bmb_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from bmb_cli.main import _relative_time
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `bmb sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from bmb_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `bmb sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.bmb/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``bmb --resume <id>``
        ...
        saved_dir = get_bmb_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"bmb_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: bmb --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from bmb_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `bmb model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from bmb_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from bmb_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from bmb_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        ...
        custom_provs = None
        try:
            from bmb_cli.config import get_compatible_custom_providers, load_config
            cfg = load_config()
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from bmb_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to bmb_cli.curator so the CLI and the `bmb curator`
        ...
        try:
            from bmb_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to bmb_cli.skills_hub."""
        from bmb_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_bmb_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_bmb_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in bmb_cli/commands.py instead of touching every dispatch site.
        from bmb_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in ("quit", "exit"):
            ...
            from bmb_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_bmb_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from bmb_cli.commands import COMMANDS
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from bmb_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_bmb_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from bmb_cli.config import load_config
        from bmb_cli.colors import Colors as _Colors
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from bmb_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from bmb_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            for quiet_logger in ('tools', 'run_agent', 'trajectory_compressor', 'cron', 'bmb_cli'):
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from bmb_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.bmb/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "bmb_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _bmb_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  bmb --resume {self.session_id}")
            if session_title:
                print(f"  bmb -c \"{session_title}\"")
            print()
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from bmb_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from bmb_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to BlackMagicBox Encover Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to BlackMagicBox Encover Agent! Type your message or /help for commands."
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from bmb_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from bmb_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "BlackMagicBox Encover Agent")
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: bmb setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    BlackMagicBox Encover Agent CLI - Interactive AI Assistant
