...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or bmb -c.",
    ...
    "Ctrl+Z suspends BMB to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "bmb -c resumes your most recent CLI session. bmb -c \"project name\" resumes by title.",
    "bmb -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "bmb -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "bmb chat -t web,terminal enables only specific toolsets for a focused session.",
    "bmb chat -s github-pr-workflow preloads a skill at launch.",
    "bmb chat -q \"query\" runs a single non-interactive query and exits.",
    "bmb chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "bmb chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "bmb --yolo bypasses all dangerous command approval prompts for the entire session.",
    "bmb chat --source telegram tags the session for filtering in bmb sessions list.",
    "bmb -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "bmb doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "bmb dump outputs a compact setup summary — great for bug reports.",
    "bmb config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "bmb config edit opens config.yaml in your default editor.",
    "bmb config check scans for missing or stale configuration options.",
    "bmb sessions browse opens an interactive session picker with search.",
    "bmb sessions stats shows session counts by platform and database size.",
    "bmb sessions prune --older-than 30 cleans up old sessions.",
    "bmb skills search react --source skills-sh searches the skills.sh public directory.",
    "bmb skills check scans installed hub skills for upstream updates.",
    "bmb skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "bmb skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "bmb mcp add github --command npx adds MCP servers from the command line.",
    "bmb mcp serve runs BMB itself as an MCP server for other agents.",
    "bmb auth add lets you add multiple API keys for credential pool rotation.",
    "bmb completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "bmb logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "bmb backup creates a zip backup of your entire BMB home directory.",
    "bmb profile create coder creates an isolated profile that becomes its own command.",
    "bmb profile create work --clone copies your current config and keys to a new profile.",
    "bmb update syncs new bundled skills to ALL profiles automatically.",
    "bmb gateway install sets up BMB as a system service (systemd/launchd).",
    "bmb memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "bmb webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: bmb tools disables unused tools, bmb skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "bmb models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as bmb -w).",
    ...
    "BMB_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call BMB tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'bmb profile create coder' creates the 'coder' command.",
    "bmb profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, BMB prints a resume command with session ID and stats.",
    "bmb sessions export backup.jsonl exports all sessions for backup or analysis.",
    "bmb -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "bmb skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .bmb/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: bmb cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "BMB runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "bmb gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to BMB via a local macOS server.",
    ...
    "Context files (.bmb.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.bmb/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "BMB loads project context from .bmb.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — bmb tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.bmb/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "bmb chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "bmb plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "bmb acp runs BMB as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "BMB_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "bmb login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "bmb status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.bmb/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make BMB your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.bmb/SOUL.md to customize.",
    ...
    "Before a gateway session reset, BMB auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.bmb/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at BMB_ENCOVER_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "BMB_ENCOVER_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in BMB_ENCOVER_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.bmb/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'bmb profile create ls' would be rejected.",
    "bmb profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "bmb claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "BMB_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via bmb tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.bmb/.env.",
    "Skills from trusted repos (Encover) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.bmb/skills/.hub/quarantine/ holds skills pending security review.",
]
