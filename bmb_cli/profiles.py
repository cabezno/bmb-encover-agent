...
Profiles live under ``~/.bmb/profiles/<name>/`` by default.

The "default" profile is ``~/.bmb`` itself — backward compatible,
zero migration needed.

Usage::

    bmb profile create coder          # fresh profile + bundled skills
    bmb profile create coder --clone  # also copy config, .env, SOUL.md, skills
    bmb profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    bmb -p coder chat                 # or via flag
    bmb profile use coder             # set as sticky default
    bmb profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See bmb_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.bmb`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.bmb) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "bmb-encover",         # repo checkout (multi-GB)
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the bmb root, NOT to the current BMB_ENCOVER_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where BMB_ENCOVER_HOME points outside
    ``~/.bmb``, profiles live under ``BMB_ENCOVER_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_bmb_home() / "profiles"


def _get_default_bmb_home() -> Path:
    """Return the default (pre-profile) BMB_ENCOVER_HOME path.

    In standard deployments this is ``~/.bmb``.
    In Docker/custom deployments where BMB_ENCOVER_HOME is outside ``~/.bmb``
    (e.g. ``/opt/data``), returns BMB_ENCOVER_HOME directly.
    """
    from bmb_constants import get_default_bmb_root
    return get_default_bmb_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_bmb_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its BMB_ENCOVER_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_bmb_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, bmb subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_bmb_home()
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.bmb)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.bmb/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches BMB_ENCOVER_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "BMB_ENCOVER_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.bmb).\n"
            "To remove everything, use: bmb uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set BMB_ENCOVER_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("BMB_ENCOVER_HOME")
    try:
        os.environ["BMB_ENCOVER_HOME"] = str(profile_dir)
        from bmb_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["BMB_ENCOVER_HOME"] = old_home
        elif "BMB_ENCOVER_HOME" in os.environ:
            del os.environ["BMB_ENCOVER_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.bmb/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: bmb profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from BMB_ENCOVER_HOME.

    Returns ``"default"`` if BMB_ENCOVER_HOME is not set or points to ``~/.bmb``.
    Returns the profile name if BMB_ENCOVER_HOME points into ``~/.bmb/profiles/<name>``.
    Returns ``"custom"`` if BMB_ENCOVER_HOME is set to an unrecognized path.
    """
    from bmb_constants import get_bmb_home
    bmb_home = get_bmb_home()
    resolved = bmb_home.resolve()

    default_resolved = _get_default_bmb_home().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: bmb profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.bmb itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.bmb). "
            "Specify a different name: bmb profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="bmb_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_bmb_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for bmb profile names."""
    return '''# BlackMagicBox Encover Agent profile completion
# Add to ~/.bashrc: eval "$(hermes completion bash)"

_hermes_profiles() {
    local profiles_dir="$HOME/.bmb/profiles"
...
def generate_zsh_completion() -> str:
    """Generate a zsh completion script for bmb profile names."""
    return '''#compdef hermes
# BlackMagicBox Encover Agent profile completion
# Add to ~/.zshrc: eval "$(hermes completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.bmb/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.bmb/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a BMB_ENCOVER_HOME path string.

    Called early in the CLI entry point, before any bmb modules
    are imported, to set the BMB_ENCOVER_HOME environment variable.
    """
