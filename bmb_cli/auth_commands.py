...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:bmb_pkce",
    ...
    if provider == "nous":
        ...
        # ~/.bmb/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `bmb --profile <name>
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `bmb auth
    ...
    raise SystemExit(f"`bmb auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `bmb auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `bmb auth` is called bare."""
