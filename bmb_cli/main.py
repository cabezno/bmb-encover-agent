...
Usage:
    bmb                     # Interactive chat (default)
    bmb chat                # Interactive chat
    bmb gateway             # Run gateway in foreground
    bmb gateway start       # Start gateway as service
    bmb gateway stop        # Stop gateway service
    bmb gateway status      # Show gateway status
    bmb gateway install     # Install gateway service
    bmb gateway uninstall   # Uninstall gateway service
    bmb setup               # Interactive setup wizard
    bmb logout              # Clear stored authentication
    bmb status              # Show status of all components
    bmb cron                # Manage cron jobs
    bmb cron list           # List cron jobs
    bmb cron status         # Check if cron scheduler is running
    bmb doctor              # Check configuration and dependencies
    bmb honcho setup                    # Configure Honcho AI memory integration
    bmb honcho status                   # Show Honcho config and connection status
    bmb honcho sessions                 # List directory → session name mappings
    bmb honcho map <name>               # Map current directory to a session name
    bmb honcho peer                     # Show peer names and dialectic settings
    bmb honcho peer --user NAME         # Set user peer name
    bmb honcho peer --ai NAME           # Set AI peer name
    bmb honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    bmb honcho mode                     # Show current memory mode
    bmb honcho mode [hybrid|honcho|local]  # Set memory mode
    bmb honcho tokens                   # Show token budget settings
    bmb honcho tokens --context N       # Set session.context() token cap
    bmb honcho tokens --dialectic N     # Set dialectic result char cap
    bmb honcho identity                 # Show AI peer identity representation
    bmb honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    bmb honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hermes + Honcho
    bmb version             Show version
    bmb update              Update to latest version
    bmb uninstall           Uninstall BlackMagicBox Encover Agent
    bmb acp                 Run as an ACP server for editor integration
    bmb sessions browse     Interactive session picker with search

    bmb claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (bmb tools, bmb setup, bmb model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'bmb {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any bmb module import.
#
# Many modules cache BMB_ENCOVER_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("BMB_ENCOVER_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.bmb/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set BMB_ENCOVER_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors bmb_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If BMB_ENCOVER_HOME is already set and no explicit flag was given, trust it.
    # This lets child processes (relaunch, subprocess) inherit the parent's
    # profile choice without having to pass --profile again.
    if profile_name is None and os.environ.get("BMB_ENCOVER_HOME"):
        return

    # 2. If no flag, check active_profile in the bmb root
    if profile_name is None:
        try:
            from bmb_constants import get_default_bmb_root

            active_path = get_default_bmb_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set BMB_ENCOVER_HOME
    if profile_name is not None:
        try:
            from bmb_cli.profiles import resolve_profile_env

            bmb_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent bmb from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["BMB_ENCOVER_HOME"] = bmb_home
...
# Load .env from ~/.bmb/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from bmb_cli.config import get_bmb_home
from bmb_cli.env_loader import load_bmb_dotenv

load_bmb_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE bmb_logging imports agent.redact (which snapshots the flag at
...
try:
    if "HERMES_REDACT_SECRETS" not in os.environ:
        ...
        _cfg_path = get_bmb_home() / "config.yaml"
        if _cfg_path.exists():
...
# Initialize centralized file logging early — all `bmb` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from bmb_logging import setup_logging as _setup_logging
...
try:
    from bmb_cli.config import load_config as _load_config_early
    from bmb_constants import apply_ipv4_preference as _apply_ipv4
...
from bmb_cli import __version__, __release_date__
from bmb_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from bmb_cli.config import get_env_path, get_bmb_home, load_config
    from bmb_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from bmb_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_bmb_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from bmb_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_bmb_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from bmb_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, bmb_bin
        cli_args: the original CLI arguments (everything after 'bmb')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    bmb_bin = container_info["bmb_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo bmb {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo bmb {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  bmb --tui --resume {target}")
    if title:
        print(f'  bmb --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @bmb/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@bmb" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _bmb_ink_bundle_stale(tui_dir):
        return True
...
def _bmb_ink_bundle_stale(tui_dir: Path) -> bool:
...
def _ensure_tui_node() -> None:
    ...
    bmb_home = os.environ.get("BMB_ENCOVER_HOME") or str(Path.home() / ".hermes")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "BMB_ENCOVER_HOME": bmb_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(bmb_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from bmb_cli.oneshot import _normalize_toolsets
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'bmb sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'bmb setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from bmb_cli.banner import prefetch_update_check
    ...
    # user's ~/.bmb/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["BMB_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from bmb_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from bmb_cli.config import get_env_value, save_env_value
    ...
    session_dir = get_bmb_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: bmb gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  bmb gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ BlackMagicBox Encover Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  bmb gateway")
        ...
        print()
        print("  Or install as a service: bmb gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'bmb whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from bmb_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``bmb model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from bmb_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from bmb_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from bmb_cli.providers import resolve_provider_full
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'bmb model' for "
                "available providers, or run 'bmb doctor' to diagnose config "
                "issues."
            )
    ...
    from bmb_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from bmb_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.bmb/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.bmb/.env if the active provider is not 'custom'.
    ...
    """
    from bmb_cli.config import get_env_value, save_env_value, load_config
...
# `bmb model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `bmb model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from bmb_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``bmb model`` flow, then route aux tasks to them here.
    """
    from bmb_cli.config import load_config
    from bmb_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from bmb_cli.auth import _prompt_model_selection
    from bmb_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from bmb_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from bmb_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from bmb_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from bmb_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from bmb_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from bmb_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `bmb auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from bmb_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from bmb_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from bmb_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from bmb_cli.auth import _save_model_choice, deactivate_provider
    from bmb_cli.config import get_env_value, load_config, save_config
    ...
    from bmb_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `bmb model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from bmb_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from bmb_cli.config import get_env_value, save_env_value, load_config, save_config
    from bmb_cli import azure_detect
    import getpass
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from bmb_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from bmb_cli.config import save_env_value, load_config, save_config
    from bmb_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from bmb_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from bmb_cli.copilot_auth import validate_copilot_token
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so bmb knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        ...
        from bmb_cli.models import fetch_lmstudio_models
        ...
        from bmb_cli.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from bmb_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'bmb model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    ...
    from bmb_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from bmb_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from bmb_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from bmb_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from bmb_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from bmb_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``bmb slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: bmb slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `bmb slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from bmb_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from bmb_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from bmb_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from bmb_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from bmb_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from bmb_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from bmb_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from bmb_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from bmb_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from bmb_cli.backup import run_import

    run_import(args)


def cmd_pair(args):
    """Vincular dispositivos via QR (app mobile/desktop)."""
    from bmb_cli.pair_cmd import cmd_pair as _pair
    import sys
...
def cmd_version(args):
    ...
    try:
        from bmb_cli.banner import check_for_updates
        from bmb_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall BlackMagicBox Encover Agent."""
    _require_tty("uninstall")
    from bmb_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``bmb update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``bmb_cli/web_dist/`` (per vite.config.ts
    outDir: "../bmb_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "bmb_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``bmb update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``bmb web``).
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``bmb dashboard`` processes other than ourselves.

    ``bmb dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``bmb update`` replaces files on disk, the running
    ...
    patterns = [
        "bmb dashboard",
        "bmb_cli.main dashboard",
        "bmb_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `bmb_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `bmb update`.
    ...
    print("  Preview now:  bmb curator run --dry-run")
    print("  Pause it:     bmb curator pause")
    print("  Docs:         https://bmb-encover.blackmagicbox.com/docs/user-guide/features/curator")


def _kill_stale_dashboard_processes(
    reason: str = "the running backend no longer matches the updated frontend",
) -> None:
    """Kill running ``bmb dashboard`` processes.

    Called at the end of ``bmb update`` (default ``reason``) and also
    from ``bmb dashboard --stop`` (which overrides ``reason``).  The
...
def _update_via_zip(args):
    """Update BlackMagicBox Encover Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/Encover/bmb-encover/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
        zip_path = os.path.join(tmp_dir, f"bmb-encover-{branch}.zip")
        ...
        # GitHub ZIPs extract to bmb-encover-<branch>/
        extracted = os.path.join(tmp_dir, f"bmb-encover-{branch}")
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes bmb completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `bmb update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Encover/bmb-encover.git",
    "git@github.com:Encover/bmb-encover.git",
    "https://github.com/Encover/bmb-encover",
    "git@github.com:Encover/bmb-encover",
}
OFFICIAL_REPO_URL = "https://github.com/Encover/bmb-encover.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_bmb_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_bmb_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from Encover/bmb-encover.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Encover/bmb-encover.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Encover/bmb-encover.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``bmb update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from bmb_constants import get_default_bmb_root

    default_home = get_default_bmb_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "bmb-encover[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``bmb update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.bmb/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``bmb update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``bmb update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.bmb/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``bmb update --gateway``) the update is already
    ...
    try:
        ...
        # bmb_cli.config.get_bmb_home to simulate setup failure.
        from bmb_cli.config import get_bmb_home as _get_bmb_home

        logs_dir = _get_bmb_home() / "logs"
...
def _cmd_update_check():
    """Implement ``bmb update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``bmb update`` without requiring a reinstall.  The
    ...
    ``bmb`` prints ``command not found`` even though the symlink lives
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/bmb-encover).
    ...
    path_comment = (
        "# BlackMagicBox Encover Agent — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of BMB_ENCOVER_HOME before running the update.
    ...
    BMB_ENCOVER_HOME directories.  The ``--backup`` flag on ``bmb update``
    ...
    try:
        from bmb_cli.backup import create_pre_update_backup
    except Exception as exc:
    ...
    # Render path using display_bmb_home so the user sees ~/.bmb/...
    try:
        from bmb_constants import get_bmb_home, display_bmb_home
        home = get_bmb_home()
        try:
            display_path = f"{display_bmb_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  bmb import {out_path}")
...
def cmd_update(args):
    """Update BlackMagicBox Encover Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from bmb_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update BlackMagicBox Encover Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating BlackMagicBox Encover Agent...")
    print()
    ...
    try:
        ...
        # the old bytecode (e.g. get_bmb_home added to bmb_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload bmb_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_bmb_home() added since the last release.
        try:
            import importlib
            import bmb_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit BMB_ENCOVER_HOME so
        # it is not affected by sync_skills()'s module-level BMB_ENCOVER_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's BMB_ENCOVER_HOME env var points at the default or a named profile.
        try:
            from bmb_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if assume_yes:
                ...
                print(
                    "    Run 'bmb config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if (gateway_mode or assume_yes) and missing_env:
                    print("  ℹ API keys require manual entry: bmb config migrate")
            else:
                print()
                print("Skipped. Run 'bmb config migrate' later to configure.")
        else:
        ...
        # When running as ``bmb update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_bmb_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all bmb-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. bmb-gateway.service or bmb-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: bmb gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: bmb -p <profile> gateway run  for each profile)"
                        )
        ...
        # current bmb-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `bmb update` surfaces the issue until the user migrates.
        try:
            from bmb_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print("  bmb-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    bmb gateway migrate-legacy")
                print()
        ...
        print("  bmb model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``bmb -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from bmb_constants import display_bmb_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `bmb profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_bmb_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → bmb -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.bmb)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  bmb profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     bmb -p {name} chat")
                else:
        ...
        from bmb_cli.profiles import profile_exists
        ...
        from bmb_cli.profiles import rename_profile
        ...
        from bmb_cli.profiles import export_profile
        ...
        from bmb_cli.profiles import import_profile
...
def _report_dashboard_status() -> int:
    """Print ``bmb dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``bmb dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No bmb dashboard processes running.")
        return 0

    print(f"{len(pids)} bmb dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No bmb dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `bmb update`.
    ...
    from bmb_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from bmb_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from bmb_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for bmb CLI."""
    from bmb_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from bmb_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://bmb-encover.blackmagicbox.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `bmb model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "units (bmb-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure BlackMagicBox Encover Agent with an interactive wizard. "
        "Run a specific section: bmb setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'bmb setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_manifest.add_argument(
        ...
        help="Write manifest to a file instead of stdout. With no PATH "
             "writes to $BMB_ENCOVER_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of BlackMagicBox Encover Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.bmb/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.bmb/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from bmb_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.bmb/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.bmb/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with BlackMagicBox Encover Agent setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for BlackMagicBox Encover Agent. Use 'bmb debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    bmb debug share              Upload debug report and print URL
    bmb debug share --lines 500  Include more log lines
    bmb debug share --expire 30  Keep paste for 30 days
    bmb debug share --local      Print report locally (no upload)
    bmb debug share --no-redact  Disable upload-time secret redaction
    bmb debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'bmb debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        ...
        "skills, sessions, and data (excludes the bmb-encover codebase). "
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage BlackMagicBox Encover Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from bmb_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.bmb/skills/.bundled_manifest) "
            "so future 'bmb update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from bmb_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from bmb_cli.skills_hub import skills_command
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `bmb plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from bmb_cli.plugins_cmd import plugins_command
    ...
    try:
        from bmb_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_bmb_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_bmb_home()}/memories/\n")
        else:
            from bmb_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'bmb tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # bmb tools disable <name...> [--platform cli]
    ...
    # bmb tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from bmb_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from bmb_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'bmb mcp add' to connect to a new server, or\n"
            "'bmb mcp serve' to expose Hermes conversations over MCP."
        ),
    )
    ...
    def cmd_mcp(args):
        from bmb_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_bmb_home() / "sessions"
            ...
            # Launch bmb --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from bmb_cli.relaunch import relaunch
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.bmb/ (by default a "
        "single restore-point archive is written to ~/.bmb/backups/ "
        "before apply; restorable with 'bmb import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from bmb_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update BlackMagicBox Encover Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'bmb config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall BlackMagicBox Encover Agent",
        description="Remove BlackMagicBox Encover Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run BlackMagicBox Encover Agent as an ACP (Agent Client Protocol) server",
        description="Start BlackMagicBox Encover Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch BlackMagicBox Encover Agent as an ACP server."""
        try:
    ...
    # =========================================================================
    # pair command — Vincular dispositivos via QR
    # =========================================================================
    pair_parser = subparsers.add_parser(
        "pair",
        help="Vincular dispositivos (app mobile/desktop) via QR",
        description="Genera un QR para vincular la app BMB con esta PC.",
    )
    pair_parser.add_argument(
        "action", nargs="?", default="show",
        choices=["show", "list", "revoke", "image"],
        help="Acción: show (default, muestra QR), list, revoke <id>, image",
    )
    pair_parser.add_argument(
        "device_id", nargs="?", default="",
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the BlackMagicBox Encover Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `bmb --tui` via PTY/WebSocket). "
    ...
    # for `bmb dashboard` cmdlines and SIGTERM them directly — the same
    # path `bmb update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running bmb dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running bmb dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    bmb logs                    Show last 50 lines of agent.log
    bmb logs -f                 Follow agent.log in real time
    bmb logs errors             Show last 50 lines of errors.log
    bmb logs gateway -n 100     Show last 100 lines of gateway.log
    bmb logs --level WARNING    Only show WARNING and above
    bmb logs --session abc123   Filter by session ID
    bmb logs --component tools  Only show tool-related lines
    bmb logs --since 1h         Lines from the last hour
    bmb logs --since 30m -f     Follow, starting from 30 min ago
    bmb logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``bmb -c Pokemon Agent Dev`` → ``bmb -c 'Pokemon Agent Dev'``
    ...
    from bmb_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'bmb -c model' where 'model' is consumed as the
    ...
    if (
        ...
        try:
            from bmb_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
    ...
    if getattr(args, "oneshot", None):
        from bmb_cli.oneshot import run_oneshot
