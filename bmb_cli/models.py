...
Add, remove, or reorder entries here — both `bmb setup` and
`bmb` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `bmb model`
    flow without maintaining a separate static list.
    """
    from bmb_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $BMB_ENCOVER_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://portal.blackmagicbox.com").rstrip("/")
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from bmb_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.blackmagicbox.com"
...
# this list:  bmb model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `bmb model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `bmb model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Nous Portal",              "Nous Portal (Encover subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``bmb model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``bmb auth add openai-codex`` without
    ``bmb model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    bmb-encover is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from bmb_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — bmb-encover
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from bmb_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from bmb_constants import AI_GATEWAY_BASE_URL
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.blackmagicbox.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``bmb model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from bmb_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from bmb_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``bmb auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.bmb/.env``.
    ...
    """
    try:
        from bmb_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from bmb_cli.auth import read_credential_pool
        from bmb_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from bmb_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_bmb_home() / "ollama_cloud_models_cache.json"
