...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``bmb kanban boards
...
# enough that kebab-case names like ``atm10-server`` or ``bmb-encover``
...
def kanban_home() -> Path:
    ...
    2. ``get_default_bmb_root()``, which already returns ``<root>``
       ...
       ``BMB_ENCOVER_HOME`` directly for Docker / custom deployments.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``bmb kanban boards switch <slug>``
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``bmb kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``bmb kanban log`` unambiguous even when multiple
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``bmb kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from bmb_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``bmb --skills ...`` alongside the built-in
    ...
    # `bmb --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``bmb kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``bmb kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``bmb kanban complete <id>``) works without requiring
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``bmb kanban
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``bmb -p <profile> chat -q ...`` subprocess.
    ...
    # `bmb kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`bmb` executable not found on PATH. "
            "Install BlackMagicBox Encover Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``bmb kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    Reads ``~/.bmb/profiles/`` directly so this module has no import
    dependency on ``bmb_cli.profiles`` (which pulls in a large chunk
    ...
    try:
        from bmb_constants import get_default_bmb_root
        home = get_default_bmb_root() / "profiles"
    except Exception:
        return []
    if not home.is_dir():
        return []
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``bmb kanban assignees`` for the terminal.
