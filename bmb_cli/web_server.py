"""
BlackMagicBox Encover Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m bmb_cli.main web          # Start on http://127.0.0.1:9119
    python -m bmb_cli.main web --port 8080
"""
...
# streamed to a per-action log file under ``~/.bmb/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_bmb_home() / "logs"
...
def _spawn_bmb_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``bmb <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``bmb_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "bmb_cli.main", *subcommand]
...
async def restart_gateway():
    """Kick off a ``bmb gateway restart`` in the background."""
    try:
        proc = _spawn_bmb_action(["gateway", "restart"], "gateway-restart")
...
# in bmb_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.bmb/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``bmb auth add <provider>`` command so the dashboard
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "bmb auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "bmb auth add nous",
        "docs_url": "https://portal.blackmagicbox.com",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "bmb auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "bmb auth add qwen-oauth",
    ...
    },
    {
        ...
        "name": "MiniMax (OAuth)",
        "flow": "pkce",
        "cli_command": "bmb auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from bmb_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("bmb_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from bmb_cli.auth import clear_provider_auth
...
#          → persists to ~/.bmb/.anthropic_oauth.json AND credential pool
...
# Guarded so bmb web still starts if anthropic_adapter is unavailable;
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from bmb_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from bmb_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from bmb_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from bmb_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_bmb_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from bmb_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_bmb_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from bmb_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "bmb setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from bmb_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from bmb_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``bmb --tui`` binary the CLI uses, behind
...
from bmb_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``bmb --tui`` would run.  Tests monkeypatch this
    ...
    """
    from bmb_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.bmb/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.bmb/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_bmb_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.bmb/dashboard-themes/*.yaml` ship with their full
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from bmb_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_bmb_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from bmb_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from bmb_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from bmb_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from bmb_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"bmb_dashboard_plugin_{plugin['name']}"
