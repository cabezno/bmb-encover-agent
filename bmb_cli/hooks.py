"""bmb hooks — inspect and manage shell-script hooks.

Usage::

    bmb hooks list
    bmb hooks test <event> [--for-tool X] [--payload-file F]
    bmb hooks revoke <command>
    bmb hooks doctor

Consent records live under ``~/.bmb/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.bmb/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``bmb hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: bmb hooks {list|test|revoke|doctor}")
        print("Run 'bmb hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.bmb/config.yaml.")
        print("See `bmb hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `bmb hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `bmb hooks test` and `bmb hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (BMB wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `bmb hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `bmb hooks doctor` would execute
