"""
Multi-provider authentication system for BlackMagicBox Encover Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.bmb/auth.json with cross-process file locking.
...
from bmb_cli.config import get_bmb_home, get_config_path, read_raw_config
from bmb_constants import OPENROUTER_BASE_URL
...
SPOTIFY_DOCS_URL = "https://bmb-encover.blackmagicbox.com/docs/user-guide/features/spotify"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from bmb_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.bmb/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `bmb model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.bmb/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_bmb_home() / "auth.json"
    # Seat belt: if pytest is running and BMB_ENCOVER_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch BMB_ENCOVER_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set BMB_ENCOVER_HOME to a tmp_path in your test fixture, or run "
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `bmb logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'bmb doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'bmb model' for available providers, or run 'bmb doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'bmb model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.bmb/.env.",
...
#
# Tokens live in ~/.bmb/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `bmb auth list` / `bmb status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.bmb/auth.json
# =============================================================================
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `bmb auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `bmb auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `bmb auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `bmb auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.bmb/.env, and return it.
    ...
    print("       App name:     anything (e.g. bmb-encover)")
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `bmb auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `bmb auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `bmb auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `bmb auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `bmb auth` to re-authenticate."
            )
            relogin_required = True
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``bmb auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``bmb auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `bmb auth add nous --type oauth`. Best-
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `bmb auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `bmb auth` and
    # `bmb model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `bmb logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from bmb_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from bmb_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'bmb model' or 'bmb setup' instead."""
    print("The 'bmb login' command has been removed.")
    print("Use 'bmb auth' to manage credentials,")
    print("'bmb model' to select a provider, or 'bmb setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.bmb/auth.json."""
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `bmb model` and select "
            "MiniMax (OAuth).",
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `bmb model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `bmb model` again to switch to Nous Portal.")
            return
