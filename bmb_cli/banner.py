...
# (e.g. nix-built bmb — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/Encover/bmb-encover.git"
...
_RELEASE_URL_BASE = "https://github.com/Encover/bmb-encover/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical Encover/bmb-encover repo (forks don't get a link).
    """
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Encover[/]")
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from bmb_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else BMB_ENCOVER_AGENT_LOGO
        console.print(_logo)
        console.print()
