"""bmb webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    bmb webhook subscribe <name> [options]
    bmb webhook list
    bmb webhook remove <name>
    bmb webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.bmb/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _bmb_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: bmb gateway run
"""
...
def webhook_command(args):
    """Entry point for 'bmb webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: bmb webhook {subscribe|list|remove|test}")
        print("Run 'bmb webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (bmb gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: bmb webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from bmb webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (bmb gateway run)")
