...
Also works when ``bmb`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def resolve_bmb_bin() -> Optional[str]:
    """Find the bmb entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("bmb")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m bmb_cli.main``.
    """
    argv0 = sys.argv[0]
    ...
    *,
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_bmb_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "bmb_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh bmb invocation."""
