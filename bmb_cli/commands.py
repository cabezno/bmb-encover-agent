...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.bmb/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update BlackMagicBox Encover Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    """
    try:
        from bmb_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``bmb skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``bmb skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``bmb skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/bmb <verb>`` subcommand.
...
def slack_app_manifest(request_url: str = "https://bmb-encover.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /bmb handler.

    Maps both canonical names and aliases so /bmb bg do stuff works
    the same as /bmb background do stuff.

    Plugin-registered slash commands are included so ``/bmb <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from bmb_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
