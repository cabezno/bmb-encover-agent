"""
Single source of truth for provider identity in BlackMagicBox Encover Agent.
...
2. **BMB overlays** — transport type, auth patterns, aggregator flags,
...
# -- BMB overlay ----------------------------------------------------------
# BMB-specific metadata that models.dev doesn't provide.
...
BMB_OVERLAYS: Dict[str, BMBOverlay] = {
    "openrouter": BMBOverlay(
    ...
    "nous": BMBOverlay(
        ...
        base_url_override="https://inference-api.blackmagicbox.com/v1",
    ),
    "openai-codex": BMBOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": BMBOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="BMB_QWEN_BASE_URL",
    ),
    "google-gemini-cli": BMBOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
    ...
    "copilot-acp": BMBOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": BMBOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": BMBOverlay(
    ...
    "alibaba-coding-plan": BMBOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "bmb", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. BMB overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + BMB overlay
    ...
    overlay = BMB_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + bmb extra
    ...
    if overlay is not None:
        # BMB-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in BMB_OVERLAYS
    if provider == "bedrock":
