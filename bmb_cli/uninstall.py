"""
BlackMagicBox Encover Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.bmb/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing bmb-encover or bmb PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# BlackMagicBox Encover Agent" comment and following line
                if '# BlackMagicBox Encover Agent' in line or '# bmb-encover' in line:
                    skip_next = True
                    continue
                if skip_next and ('bmb' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'bmb' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains bmb_cli reference)
                content = wrapper.read_text()
                if 'bmb_cli' in content or 'bmb-encover' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``bmb gateway run`` processes
    ...
    try:
        from bmb_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from bmb_cli.gateway import get_launchd_plist_path
...
def _is_default_bmb_home(bmb_home: Path) -> bool:
    """Return True when ``bmb_home`` points at the default (non-profile) root."""
    try:
        from bmb_constants import get_default_bmb_root
        return bmb_home.resolve() == get_default_bmb_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its BMB_ENCOVER_HOME directory.

    We shell out to ``bmb -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current BMB_ENCOVER_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m bmb_cli.main` so we don't depend on a `bmb`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "bmb_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's BMB_ENCOVER_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.bmb/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.bmb/ for future reinstall
    """
    project_root = get_project_root()
    bmb_home = get_bmb_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie BMB_ENCOVER_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_bmb_home(bmb_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ BlackMagicBox Encover Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {bmb_home / 'config.yaml'}")
    print(f"  Secrets: {bmb_home / '.env'}")
    print(f"  Data:    {bmb_home / 'cron/'}, {bmb_home / 'sessions/'}, {bmb_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their BMB_ENCOVER_HOME dirs. Otherwise
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.bmb/, just remove the bmb-encover subdir
            if bmb_home in project_root.parents or project_root.parent == bmb_home:
    ...
    # 5. Optionally remove ~/.bmb/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile BMB_ENCOVER_HOME dirs live under
        ...
        try:
            if bmb_home.exists():
                shutil.rmtree(bmb_home)
                log_success(f"Removed {bmb_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {bmb_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {bmb_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/Encover/bmb-encover/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using BlackMagicBox Encover Agent! ⚕")
    print()
