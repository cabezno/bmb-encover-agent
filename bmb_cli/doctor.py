"""
Doctor command for bmb CLI.

Diagnoses issues with BlackMagicBox Encover Agent setup.
"""
...
from bmb_cli.config import get_project_root, get_bmb_home, get_env_path
from bmb_constants import display_bmb_home

PROJECT_ROOT = get_project_root()
BMB_ENCOVER_HOME = get_bmb_home()
_DHH = display_bmb_home()  # user-facing display path (e.g. ~/.bmb or ~/.bmb/profiles/coder)

# Load environment variables from ~/.bmb/.env so API key checks work
...
from bmb_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.bmb/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # Check ~/.bmb/.env (primary location for user config)
    env_path = BMB_ENCOVER_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'bmb setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'bmb setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'bmb setup' to create one")
                issues.append("Run 'bmb setup' to create .env")
    
    # Check ~/.bmb/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = BMB_ENCOVER_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from bmb_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from bmb_cli.providers import (
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'bmb config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom", "openrouter"):
                try:
                    from bmb_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.bmb/.env or run 'bmb setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'bmb setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'bmb config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from bmb_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'bmb setup' to migrate config")
                else:
                    issues.append("Run 'bmb doctor --fix' or 'bmb setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'bmb doctor --fix'")
        except Exception:
            pass

        # Validate config structure (catches malformed custom_providers, etc.)
        try:
            from bmb_cli.config import validate_config_structure
    ...
    bmb_home = BMB_ENCOVER_HOME
    if bmb_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            bmb_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = bmb_home / subdir_name
    ...
    memories_dir = bmb_home / "memories"
    ...
    state_db_path = bmb_home / "state.db"
    ...
    wal_path = bmb_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'bmb doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "bmb"
        ...
        if _venv_bin is None:
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/bmb → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/bmb points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/bmb → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/bmb — run 'bmb doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/bmb exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/bmb not found",
                    "(hermes command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/bmb → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/bmb symlink — run 'bmb doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'bmb-encover[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'bmb-encover[vercel]'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'bmb config set model.provider <provider>' to switch providers, "
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'bmb setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = BMB_ENCOVER_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: bmb skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = BMB_ENCOVER_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: bmb memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: bmb memory setup")
                issues.append("No Honcho API key — run 'bmb memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run bmb memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: bmb memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: bmb memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from bmb_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "bmb -p" in content:
                            _m = _re.search(r"bmb -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'bmb doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
