...
Usage examples::
    ...
    bmb logs                    # last 50 lines of agent.log
    bmb logs -f                 # follow agent.log in real time
    bmb logs errors             # last 50 lines of errors.log
    bmb logs gateway -n 100    # last 100 lines of gateway.log
    bmb logs --level WARNING    # only WARNING+ lines
    bmb logs --session abc123   # filter by session ID substring
    bmb logs --component tools  # only tool-related lines
    bmb logs --since 1h         # lines from the last hour
    bmb logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if follow:
        print(f"--- {display_bmb_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_bmb_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_bmb_home()}/logs/")
        return

    print(f"Log files in {display_bmb_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'bmb chat' to generate logs)")
