"""``bmb debug`` debug tools for BlackMagicBox Encover Agent.

Currently supports:
    bmb debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.bmb/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.bmb/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``bmb debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``bmb debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, BMB version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `bmb debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "bmb-encover/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``bmb debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.bmb/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``bmb debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "bmb-encover/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from bmb_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_bmb_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``bmb dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``bmb dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "bmb debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  bmb debug delete <url>")

    print(f"\nShare these links with the BMB team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: bmb debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'bmb debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``bmb debug`` call.
    ...
    # best-effort — any failure is swallowed so ``bmb debug`` stays
