"""
Gateway subcommand for bmb CLI.

Handles: bmb gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_bmb_home is imported lazily at call sites to avoid ImportError
# when bmb_constants is cached from a pre-update version during `bmb update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "bmb-gateway*",
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``bmb gateway status`` from falsely counting the ``bmb`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``bmb gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "bmb_cli.main gateway",
        "bmb_cli.main --profile",
        "bmb_cli.main -p",
        "bmb_cli/main.py gateway",
        "bmb_cli/main.py --profile",
        "bmb_cli/main.py -p",
        "bmb gateway",
        "gateway/run.py",
    ]
    current_home = str(get_bmb_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"BMB_ENCOVER_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "BMB_ENCOVER_HOME=" in command and f"BMB_ENCOVER_HOME={current_home}" not in command:
            return False
        return True
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from bmb_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "bmb_cli.main"]
    if profile != "default":
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}bmb gateway status\n"
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `bmb gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``bmb gateway status`` output so users with
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (BMB_ENCOVER_HOME-scoped).
...
from bmb_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "bmb-gateway"
SERVICE_DESCRIPTION = "BlackMagicBox Encover Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current BMB_ENCOVER_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (BMB_ENCOVER_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from bmb_constants import get_default_bmb_root
    home = get_bmb_home().resolve()
    default = get_default_bmb_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary BMB_ENCOVER_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(bmb_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when BMB_ENCOVER_HOME is a named profile.

    For ``~/.bmb/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        bmb_home: Optional explicit BMB_ENCOVER_HOME path. Defaults to the current
            ``get_bmb_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from bmb_constants import get_default_bmb_root
    home = Path(bmb_home or str(get_bmb_home())).resolve()
    default = get_default_bmb_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this BMB_ENCOVER_HOME.

    Default ``~/.bmb`` returns ``bmb-gateway`` (backward compatible).
    Profile ``~/.bmb/profiles/coder`` returns ``bmb-gateway-coder``.
    Any other BMB_ENCOVER_HOME appends a short hash for uniqueness.
    """
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``bmb-gateway-coder.service`` and unrelated third-party
      ``bmb-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``bmb.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("  These run alongside the current bmb-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    bmb gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: bmb gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo bmb gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    bmb gateway uninstall")
    print_info("    sudo bmb gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            ...
            if run_as_user:
                ...
                print_info("  After setup, run: sudo bmb gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo bmb gateway start --system")
            return scope, False
...
def _bmb_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current BMB_ENCOVER_HOME to the equivalent under a target user's home.
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        bmb_home = _bmb_home_for_target_user(home_dir)
        profile_arg = _profile_arg(bmb_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m bmb_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="BMB_ENCOVER_HOME={bmb_home}"
Restart=always
RestartSec=60
...
"""
    ...
    bmb_home = str(get_bmb_home().resolve())
    profile_arg = _profile_arg(bmb_home)
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}bmb gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}bmb gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}bmb gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `bmb gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}bmb gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}bmb gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}bmb gateway start{scope_flag}")
...
<dict>
    ...
    <dict>
        ...
        <key>BMB_ENCOVER_HOME</key>
        <string>{bmb_home}</string>
    </dict>
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  bmb gateway status             # Check status")
...
def launchd_stop():
    ...
    # `bmb gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_bmb_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # /restart) rather than through `bmb gateway restart` which already
    ...
    # `bmb gateway start/restart` — leaving the gateway vulnerable to
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @bmb:matrix.example.org"},
...
def _all_platforms() -> list[dict]:
    ...
    ``bmb setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.bmb/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_bmb_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'bmb pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: bmb pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'bmb gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from bmb_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from bmb_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `bmb gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_bmb_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `bmb pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `bmb pairing approve`.")
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``bmb_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from bmb_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.bmb/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.bmb/.env: {', '.join(required)}")
    else:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: bmb gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  bmb gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: bmb gateway run")
                print_info("  For persistence:   tmux new -s bmb 'bmb gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: bmb gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup bmb gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: bmb gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'bmb gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: bmb gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: bmb gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s bmb 'bmb gateway run'")
                print()
            ...
            print()
            print("  bmb gateway run                              # direct foreground")
            print("  tmux new -s bmb 'bmb gateway run'         # persistent via tmux")
            print("  nohup bmb gateway run > ~/.bmb/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: bmb gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: bmb gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: bmb gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: bmb gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: bmb gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  bmb gateway install")
                    print("  sudo bmb gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  bmb gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup bmb gateway run > ~/.bmb/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s bmb 'bmb gateway run'         # persistent via tmux")
                    print("  nohup bmb gateway run > ~/.bmb/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  bmb gateway install  # Install as user service")
                    print("  sudo bmb gateway install --system  # Install as boot-time system service")
