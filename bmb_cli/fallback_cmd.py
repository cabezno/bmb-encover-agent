...
Subcommands:
  bmb fallback [list]   Show the current fallback chain (default when no subcommand)
  bmb fallback add      Pick provider + model via the same picker as `bmb model`,
                           then append the selection to the chain
  bmb fallback remove   Pick an entry to delete from the chain
  bmb fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.bmb/config.yaml`` (top-level, list of
...
def _snapshot_auth_active_provider() -> Any:
    ...
    try:
        from bmb_cli.auth import _load_auth_store
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from bmb_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `bmb model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  bmb fallback add")
        print()
        return
    ...
    print("  Docs: https://bmb-encover.blackmagicbox.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `bmb model`, then append the selection to the chain."""
    from bmb_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `bmb model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `bmb fallback list` to view, or `bmb fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``bmb fallback [subcommand]``."""
