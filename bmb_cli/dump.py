"""
Dump command for bmb CLI.

Outputs a compact, plain-text summary of the user's BMB setup
...
from bmb_cli.config import get_bmb_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``bmb dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from bmb_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(bmb_home: Path) -> int:
...
def _cron_summary(bmb_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = bmb_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"bmb_home:      {display_bmb_home()}")
    ...
    toolsets = config.get("toolsets", ["bmb-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(bmb_home)}")
    lines.append(f"  skills:             {_count_skills(bmb_home)}")
