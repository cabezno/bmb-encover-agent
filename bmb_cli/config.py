"""
Configuration management for BlackMagicBox Encover Agent.

Config files are stored in ~/.bmb/ for easy access:
- ~/.bmb/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.bmb/.env         - API keys and secrets

This module provides:
- bmb config          - Show current configuration
- bmb config edit     - Open config in editor
- bmb config set      - Set a specific value
- bmb config wizard   - Re-run setup wizard
"""
...
from bmb_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_bmb_home() / ".managed"
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade bmb-encover"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from BMB_ENCOVER_HOME/.container-mode.
    ...
    container_mode_file = get_bmb_home() / ".container-mode"
...
# Re-export from bmb_constants — canonical definition lives there.
from bmb_constants import get_bmb_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_bmb_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_bmb_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the bmb group can
    share state with the gateway service.

    The mode can be overridden via the BMB_ENCOVER_HOME_MODE environment variable
    (e.g. BMB_ENCOVER_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse BMB_ENCOVER_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("BMB_ENCOVER_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into BMB_ENCOVER_HOME if the user doesn't have one yet."""
...
def ensure_bmb_home():
    """Ensure ~/.bmb directory structure exists with secure permissions.
...
def _ensure_bmb_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"BMB_ENCOVER_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "checkpoints": {
        ...
        # ~/.bmb/checkpoints/ (one per cd'd working directory). Field
    ...
    "auxiliary": {
        ...
        # model; override via `bmb model` → auxiliary → Curator to route
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.bmb/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.bmb/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        # "hindsight", "holographic", "retaindb", "byterover", "gbrain".
    ...
    # cron daemon.
    #
    # See `bmb curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.bmb/skills/ into
        # ~/.bmb/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `bmb curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *BlackMagicBox Encover Agent*" header.
    ...
    # stored approval from ~/.bmb/shell-hooks-allowlist.json.
    ...
    # todos to ready, and fires `bmb -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     bmb-encover's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.bmb/logs/.
    ...
    # update model picker lists without shipping a bmb-encover release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://bmb-encover.blackmagicbox.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `bmb model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.bmb/state.db.
    ...
    # ``bmb update`` behaviour.
    "updates": {
        # Run a full ``bmb backup``-style zip of BMB_ENCOVER_HOME before every
        # ``bmb update``.  Backups land in ``<BMB_ENCOVER_HOME>/backups/`` and
        # can be restored with ``bmb import <path>``.  Off by default —
        # on large BMB_ENCOVER_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'bmb model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. blackmagicbox.com -> firecrawl-gateway.blackmagicbox.com",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @bmb:example.org)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'bmb-encover' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `bmb update`.  Skill-config prompting is a
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``bmb_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'bmb setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'bmb doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("BMB_ENCOVER_HOME", "~/.bmb")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$BMB_ENCOVER_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_bmb_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `bmb plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `bmb curator status` has no
    ...
    #   3. Creates `~/.bmb/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_bmb_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_bmb_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: bmb config set <key> <value>")
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _bmb_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.bmb/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.bmb/config.yaml.
    ...
    (which change ``BMB_ENCOVER_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_bmb_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — bmb auth) — OpenAI Codex
#   nous         (OAuth — bmb auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.bmb/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.bmb/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.bmb/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.bmb/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.bmb/.env and os.environ.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.bmb/.env or environment."""
...
def show_config():
    ...
    print(color("  bmb config edit     # Edit config file", Colors.DIM))
    print(color("  bmb config set <key> <value>", Colors.DIM))
    print(color("  bmb setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: bmb config set <key> <value>")
            print()
            print("Examples:")
            print("  bmb config set model anthropic/claude-sonnet-4")
            print("  bmb config set terminal.backend docker")
            print("  bmb config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'bmb config migrate' to add them")
        ...
        print("  bmb config           Show current configuration")
        print("  bmb config edit      Open config in editor")
        print("  bmb config set <key> <value>   Set a config value")
        print("  bmb config check     Check for missing/outdated config")
        print("  bmb config migrate   Update config with new options")
        print("  bmb config path      Show config file path")
        print("  bmb config env-path  Show .env file path")
        sys.exit(1)
