"""
Backup and import commands for bmb CLI.

`bmb backup` creates a zip archive of the entire ~/.bmb/ directory
(excluding the bmb-encover repo and transient files).

`bmb import` restores from a backup zip, overlaying onto the current
BMB_ENCOVER_HOME root.
"""
...
from bmb_constants import get_default_bmb_root, get_bmb_home, display_bmb_home
...
_EXCLUDED_DIRS = {
    "bmb-encover",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to bmb root) should be skipped."""
...
def run_backup(args) -> None:
    ...
    # Collect files
    print(f"Scanning {display_bmb_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(bmb_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(bmb_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(bmb_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside bmb root
            try:
    ...
    print(f"\nRestore with: bmb import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a bmb home would have
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_bmb_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (bmb_root / "config.yaml").exists()
        has_env = (bmb_root / ".env").exists()
        ...
        bmb_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            try:
                target.resolve().relative_to(bmb_root.resolve())
            except ValueError:
        ...
        profiles_dir = bmb_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # bmb_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: bmb profile list  (after installing hermes)")

        # Guidance
        print()
        if not (bmb_root / "bmb-encover").is_dir():
            print("Note: The bmb-encover codebase was not included in the backup.")
            print("  If this is a fresh install, run: bmb update")
...
# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and bmb backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to BMB_ENCOVER_HOME).
...
# — `bmb update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(bmb_home: Optional[Path] = None) -> Path:
    home = bmb_home or get_bmb_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    bmb_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(bmb_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(bmb_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for bmb backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_bmb_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, bmb_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``bmb_root`` to ``out_path``.
...
def _pre_update_backup_dir(bmb_home: Optional[Path] = None) -> Path:
    home = bmb_home or get_bmb_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of BMB_ENCOVER_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<BMB_ENCOVER_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``bmb update``) should continue even if the backup fails.
    """
    bmb_root = bmb_home or get_default_bmb_root()
    if not bmb_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(bmb_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, bmb_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `bmb claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of BMB_ENCOVER_HOME under ``backups/`` before a
    ``bmb claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``bmb import <archive>``.  Writes to
    ``<BMB_ENCOVER_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `bmb import` and the
