"""
Unified tool configuration for BlackMagicBox Encover Agent.

`bmb tools` and `bmb setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.bmb/config.yaml under
...
from bmb_cli.nous_subscription import (
...
from bmb_cli.cli_output import (  # noqa: E402 — late import block
...
# Platform-scoped toolsets: only appear in the `bmb tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``bmb tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from bmb_cli.plugins import discover_plugins, get_plugin_toolsets
...
from bmb_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_bmb_home()}/bmb-encover && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in bmb-encover directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/blackmagicbox/bmb-encover:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.bmb/config.yaml")

    elif post_setup_key == "spotify":
        # Run the full `bmb auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.bmb/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from bmb_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: bmb auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `bmb auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: bmb auth spotify")
        except Exception as exc:
        ...
        try:
            from bmb_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _get_platform_tools(
    ...
    *,
    ...
    # shipped as a bundled plugin but user must opt in via `bmb tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `bmb tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by bmb tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `bmb tools` is the user's opt-in to reconfigure tools, so treat
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `bmb tools` and `bmb setup tools`.
    ...
    print()
    from bmb_constants import display_bmb_home
    print(color(f"  Tool configuration saved to {display_bmb_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'bmb' or gateway restart.", Colors.DIM))
    print()
