# BlackMagicBox Encover Agent Has Had "Routines" Since March
...
| | Claude Code Routines | BlackMagicBox Encover Agent |
|---|---|---|
...
BlackMagicBox Encover Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install bmb-encover
bmb setup
```

Set up a scheduled task in 30 seconds:
```bash
bmb cron create "0 9 * * 1" \
...
```bash
bmb gateway setup    # enable webhooks
bmb webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [bmb-encover.blackmagicbox.com/docs/guides/automation-templates](https://bmb-encover.blackmagicbox.com/docs/guides/automation-templates)
...
*BlackMagicBox Encover Agent is built by [Encover](https://blackmagicbox.com). Open source, model-agnostic, runs on your infrastructure.*
