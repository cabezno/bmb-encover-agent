...
Uses a file-based lock (~/.bmb/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `bmb update` reloads
# the module) fail with ModuleNotFoundError for bmb_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from bmb_constants import get_bmb_home
from bmb_cli.config import load_config, _expand_env_vars
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``bmb tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from bmb_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within BMB_ENCOVER_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against BMB_ENCOVER_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    scripts_dir = get_bmb_home() / "scripts"
    ...
    # escape — scripts MUST reside within BMB_ENCOVER_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _bmb_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_bmb_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_bmb_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        try:
            load_dotenv(str(_bmb_home / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_bmb_home / ".env"), override=True, encoding="latin-1")
        ...
        try:
            import yaml
            _cfg_path = str(_bmb_home / "config.yaml")
        ...
        agent = AIAgent(
            ...
            # BMB_ENCOVER_HOME. When a workdir is configured, also inject project
...
**Job ID:** {job_id}
**Run Time:** {_bmb_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _bmb_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _bmb_now().strftime('%H:%M:%S'), len(due_jobs))
