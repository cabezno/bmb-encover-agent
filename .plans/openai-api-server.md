# OpenAI-Compatible API Server for BlackMagicBox Encover Agent
...
SSE streaming. By exposing this endpoint, bmb-encover becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  bmb-encover   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.bmb/.env`
2. Run `bmb gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with bmb-encover through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns bmb-encover as a model) |
...
Authorization: Bearer bmb-api-key-here
...
But bmb-encover sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "bmb-encover"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured bmb-encover model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of bmb-encover's core prompt
...
Once implemented, bmb-encover works as a drop-in backend for:
