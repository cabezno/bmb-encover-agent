"""
Timezone-aware clock for BMB.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``BMB_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.bmb/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — BMB never
...
from bmb_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — BMB requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("BMB_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
