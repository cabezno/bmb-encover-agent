...
t1 = kanban_create(
    ...
    tenant=os.environ.get("BMB_TENANT"),
)["task_id"]
...
>
> The dispatcher will pick up T1 and T2 now. T3 starts when both finish. You'll get a gateway ping when T4 completes. Use the dashboard or `bmb kanban tail <id>` to follow along.
...
**Tenant inheritance.** If `BMB_TENANT` is set in your env, pass `tenant=os.environ.get("BMB_TENANT")` on every `kanban_create` call so child tasks stay in the same namespace.
