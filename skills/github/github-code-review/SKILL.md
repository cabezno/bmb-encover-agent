...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.bmb/.env ] && grep -q "^GITHUB_TOKEN=" ~/.bmb/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.bmb/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from BlackMagicBox Encover Agent\",
    \"comments\": [
...
```bash
source "${BMB_ENCOVER_HOME:-$HOME/.bmb}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by BlackMagicBox Encover Agent. Code looks clean — good test coverage, no security concerns."
...
curl -s -X POST \
  ...
  -d "{
    ...
    \"body\": \"## BlackMagicBox Encover Agent Review\n\nFound 2 issues, 1 suggestion. See inline comments.\",
    \"comments\": [
...
---
*Reviewed by BlackMagicBox Encover Agent*
EOF
)"
```
