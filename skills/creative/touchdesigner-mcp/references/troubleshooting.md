...
`$BMB_ENCOVER_HOME/config.yaml` (defaults to `~/.bmb/config.yaml` when `BMB_ENCOVER_HOME` is unset)
...
Restart the BMB session for changes to take effect. The MCP connection is
