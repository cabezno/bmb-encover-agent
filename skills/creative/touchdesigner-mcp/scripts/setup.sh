...
BMB_ENCOVER_HOME_DIR="${BMB_ENCOVER_HOME:-$HOME/.bmb}"
BMB_CFG="${BMB_ENCOVER_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure BMB config has twozero_td MCP entry ──
if [[ ! -f "$BMB_CFG" ]]; then
    echo -e " ${FAIL} BMB config not found at ${BMB_CFG}"
    manual_steps+=("Create ${BMB_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$BMB_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in BMB config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to BMB config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${BMB_CFG} manually"); }
    manual_steps+=("Restart BMB session to pick up config change")
fi
