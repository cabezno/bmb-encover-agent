...
metadata:
  bmb:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, bmb-video]
...
```
BlackMagicBox Encover Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${BMB_ENCOVER_HOME:-$HOME/.bmb}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to BMB config (if missing)
...
3. **Restart BMB session** to pick up the new MCP server
