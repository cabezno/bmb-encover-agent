...
And add it to the `bmb-gateway` composite:
...
## 12. Status Display (`bmb_cli/status.py`)
...
## 13. Gateway Setup Wizard (`bmb_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ bmb_cli/ toolsets.py \
  --include="*.py" -l | sort -u
