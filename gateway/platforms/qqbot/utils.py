...
def _get_bmb_version() -> str:
    """Return the bmb-encover package version, or 'dev' if unavailable."""
    try:
        from importlib.metadata import version
        return version("bmb-encover")
    except Exception:
        return "dev"
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; BMB/<bmb_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; BMB/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    bmb_version = _get_bmb_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; BMB/{bmb_version})"
