...
# Uses get_bmb_home() so each profile gets its own Matrix store.
from bmb_constants import get_bmb_dir as _get_bmb_dir

_STORE_DIR = _get_bmb_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("BMB_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("BMB_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    ...
                    device_name="BlackMagicBox Encover Agent",
                    device_id=self._device_id or None,
                )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "BlackMagicBox Encover Agent" become "Agent".
        """
        if not body:
            return ""
