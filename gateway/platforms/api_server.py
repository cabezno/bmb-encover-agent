...
- GET  /v1/models                  — lists bmb-encover as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to bmb-encover
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_bmb_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through bmb-encover's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``bmb sessions list``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "bmb-encover"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return bmb-encover as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "bmb.api_server.capabilities",
            "platform": "bmb-encover",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``bmb.tool.progress`` with ``status: running``.
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: bmb.tool.progress`` SSE event so
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from bmb_cli.auth import has_usable_secret
