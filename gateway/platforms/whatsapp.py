...
from bmb_constants import get_bmb_dir
...
class WhatsAppAdapter(BasePlatformAdapter):
    ...
    # Default bridge location relative to the bmb-encover install
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_bmb_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Allow all users through the bridge — the Python auto-responder
            # handles tiered filtering (amigo/cliente/no_registrado).
            bridge_env["WHATSAPP_ALLOWED_USERS"] = "*"
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: bmb whatsapp")
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        import logging as _wa_log
        _wa_log.getLogger(__name__).info(
            "SEND_VOICE: chat_id=%s audio_path=%s exists=%s size=%d ext=%s",
            chat_id, audio_path,
            os.path.exists(audio_path),
            os.path.getsize(audio_path) if os.path.exists(audio_path) else -1,
            os.path.splitext(audio_path)[1] if audio_path else "none"
        )
        # NEVER send MP3 as voice to WhatsApp — it won't play as native voice
        ext = os.path.splitext(audio_path)[1].lower() if audio_path else ""
        if ext == ".mp3":
            _wa_log.getLogger(__name__).info(
                "SEND_VOICE: skipping MP3 (%s) — only .ogg/.opus supported for WhatsApp voice",
                audio_path
            )
            return SendResult(success=False, error="MP3 not supported for WhatsApp voice")
        # Only accept audio from bmb_voice/ (generated by _send_voice_reply).
        # Audio from audio_cache/ comes from the agent's text_to_speech tool and
        # is always .mp3 which WhatsApp can't play as native voice.
        if "audio_cache" in audio_path:
            _wa_log.getLogger(__name__).info(
                "SEND_VOICE: skipping audio_cache file (%s) — gateway voice reply handles it",
                audio_path
            )
            return SendResult(success=False, error="Only gateway voice reply audio accepted")
    ...
    async def _build_message_event(self, data: Dict[str, Any]) -> Optional[MessageEvent]:
        ...
        try:
            ...
            # Auto-responder: check contact database before passing to agent
            # SAFETY: never auto-respond unless explicitly enabled in .env
            import os as _os
            _wa_enabled = _os.environ.get("WHATSAPP_ENABLED", "false").lower() in ("true", "1", "yes")
            auto_response = (await self._auto_respond_if_needed(data)) if _wa_enabled else None
            _auto_skill = None  # Skill to auto-load for this session
            if auto_response is not None:
                response_text = auto_response.get("mensaje", "")
                should_suppress = auto_response.get("suprimir", False)
                if response_text:
                    chat_id = data.get("chatId", "") or data.get("from", "")
                    if chat_id:
                        await self.send(chat_id, response_text)
                if should_suppress:
                    return None  # Suppress message — we already responded
                # Otherwise: message continues to agent after auto-reply
            elif _wa_enabled and auto_response is None:
                # No auto-response sent (empty primer_mensaje) — check if we
                # should auto-load the grabarpodcast skill for this contact
                _contact_info = await self._classify_contact(data)
                if _contact_info and _contact_info.get("tipo") in ("cliente", "no_registrado"):
                    # Check if it's Adriana (asistente/admin) - skip skill, let her talk to Hermes
                    _sender_id = str(data.get("senderId") or data.get("from") or "")
                    _sender_phone = _sender_id.split("@")[0] if _sender_id else ""
                    if _sender_phone not in ("59898454869", "263557077811238"):
                        _auto_skill = "grabarpodcast-asistente"
    ...
    async def _classify_contact(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Clasifica un contacto sin enviar auto-respuesta. Devuelve tipo y nombre."""
        ...
        responder_path = "/mnt/c/Users/Pc Nasa/Desktop/PROGRAMA AUTOMATICO PUBLICACIONES/whatsapp_responder.py"
        if not os.path.exists(responder_path):
            return None
        try:
            args = ["python3", responder_path, phone]
            if sender_name:
    ...
    async def _auto_respond_if_needed(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Check if this message needs an automatic response based on the
        WhatsApp contacts database.
        
        Returns a dict with:
          - "mensaje": text to send as auto-reply (or None if no reply needed)
          - "suprimir": bool — if True, the original message is NOT passed to the agent
        
        Returns None if the message should pass through normally with no action.
        """
        import subprocess
        import json
        import os
        
        # Only for DMs, not groups
        ...
        # Extract phone number from sender ID (format: "59899244029@s.whatsapp.net")
        phone = sender_id.split("@")[0]
        if not phone:
            return None
        
        # Call the responder script
        responder_path = "/mnt/c/Users/Pc Nasa/Desktop/PROGRAMA AUTOMATICO PUBLICACIONES/whatsapp_responder.py"
        if not os.path.exists(responder_path):
            return None
        ...
        try:
            ...
            if response.get("accion") == "passthrough":
                # User is Santiago - don't respond, don't suppress
                return None
            
            response_text = response.get("mensaje", "")
            should_respond = response.get("responder", False)
            contacto_tipo = response.get("tipo", "")

            # Si el mensaje esta vacio, no hay auto-respuesta, pasa directo al agente
            if not response_text:
                return None

            # Amigos: reply but suppress message from agent (Santiago takes over)
            # Clientes / No registrados: reply but LET the message through to agent
            if response_text:
                suppress = (contacto_tipo == "amigo")
                return {"mensaje": response_text, "suprimir": suppress}
