"""
Microsoft Teams platform adapter for BlackMagicBox Encover Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        bmb_action = data.get("bmb_action", "")
        session_key = data.get("session_key", "")

        if not bmb_action or not session_key:
        ...
        choice = choice_map.get(bmb_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="bmb_approve",
                    data={**btn_data_base, "bmb_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="bmb_approve",
                    data={**btn_data_base, "bmb_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="bmb_approve",
                    data={**btn_data_base, "bmb_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="bmb_approve",
                    data={**btn_data_base, "bmb_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    from bmb_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_info,
        print_success,
    ...
    print()
    print_success("Teams configuration saved to ~/.bmb/.env")
    print_info("Install the app in Teams:  teams app install --id <teamsAppId>")
    print_info("Restart the gateway:       bmb gateway restart")
