...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :BlackMagicBox Encover Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :BlackMagicBox Encover Agent shutting down")
...
def check_requirements() -> bool:
    ...
    # bmb setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `bmb gateway setup` flow for the IRC platform.

    Lazy-imports ``bmb_cli.setup`` helpers so the plugin stays importable
    ...
    print()
    print_success("IRC configuration saved to ~/.bmb/.env")
    print_info("Restart the gateway for changes to take effect: bmb gateway restart")
