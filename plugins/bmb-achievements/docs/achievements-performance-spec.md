# BMB Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: bmb-achievements plugin
...
### `GET /api/plugins/bmb-achievements/achievements`
...
### `POST /api/plugins/bmb-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/bmb-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/bmb-achievements/overview`
...
- Persist plugin data under:
  - `~/.bmb/plugins/bmb-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.bmb/plugins/bmb-achievements/scan_checkpoint.json`
