...
Handles: bmb honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'bmb honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'bmb honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `bmb update` -- no output, no exceptions.
    """
    try:
        from bmb_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _local_config_path() -> Path:
    ...
    Always returns $BMB_ENCOVER_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_bmb_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'bmb honcho setup' again once you have a key.\n")
            return
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "bmb")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "bmb")
    ...
    try:
        ...
        bmb_config = load_config()
        bmb_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(bmb_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: bmb config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    bmb honcho status     -- show full config")
    print("    bmb honcho mode       -- change recall/observation mode")
    print("    bmb honcho tokens     -- tune context and dialectic budgets")
    print("    bmb honcho peer       -- update peer names")
    print("    bmb honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: bmb honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'bmb honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: bmb honcho map <session-name>")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: bmb honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: bmb honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("\n  Set with: bmb honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'bmb honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'bmb honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    bmb honcho identity --show        — show both peer representations")
        print("    bmb honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  bmb honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'bmb honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'bmb honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  bmb honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        for f in user_files:
            print("    bmb honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'bmb honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'bmb honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    bmb honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  bmb honcho identity <path/to/SOUL.md>")
    ...
    print()
    if not has_key:
        print("  1. bmb honcho setup              — configure API key (required)")
        print("  2. bmb honcho migrate            — re-run this walkthrough")
    else:
        print("  1. bmb honcho status             — verify Honcho connection")
        print("  2. bmb                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. bmb honcho identity --show    — verify AI peer representation")
        print("  4. bmb honcho tokens             — tune context and dialectic budgets")
        print("  5. bmb honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'bmb memory setup'...\n")
        from bmb_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``bmb honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``bmb honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to bmb memory setup)",
    )
