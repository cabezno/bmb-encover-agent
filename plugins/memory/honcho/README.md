...
Or manually:
```bash
bmb config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.bmb/.env
```
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-bmb-encover`.
...
| Command | Description |
|---------|-------------|
| `bmb honcho setup` | Full interactive setup wizard |
| `bmb honcho status` | Show resolved config for active profile |
| `bmb honcho enable` / `disable` | Toggle Honcho for active profile |
| `bmb honcho mode <mode>` | Change recall or observation mode |
| `bmb honcho peer --user <name>` | Update user peer name |
| `bmb honcho peer --ai <name>` | Update AI peer name |
