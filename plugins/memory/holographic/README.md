...
Or manually:
```bash
bmb config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.bmb-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$BMB_ENCOVER_HOME/memory_store.db` | SQLite database path |
