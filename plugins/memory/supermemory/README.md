...
```bash
bmb config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.bmb/.env
```

## Config

Config file: `$BMB_ENCOVER_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `bmb` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `bmb-{identity}` → `bmb-coder`). |
...
For a profile named `coder`, this resolves to `bmb-coder`. The default profile resolves to `bmb-default`. Without `{identity}`, all profiles share the same container.
