...
def _load_supermemory_config(bmb_home: str) -> dict:
    config = _default_config()
    config_path = Path(bmb_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, bmb_home: str) -> None:
    config_path = Path(bmb_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `bmb memory setup`.
        # All other options are documented for $BMB_ENCOVER_HOME/supermemory.json
    ...
    def save_config(self, values, bmb_home):
        ...
        _save_supermemory_config(sanitized, bmb_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from bmb_constants import get_bmb_home
        self._bmb_home = kwargs.get("bmb_home") or str(get_bmb_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._bmb_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "bmb", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "bmb_memory", "target": target, "type": "explicit_memory"},
