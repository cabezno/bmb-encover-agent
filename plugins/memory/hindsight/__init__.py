...
Or via $BMB_ENCOVER_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $BMB_ENCOVER_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_bmb_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "bmb": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "bmb"),
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``bmb-{user}`` with no user becomes ``bmb``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between BlackMagicBox Encover Agent and the User"
    ...
    def save_config(self, values, bmb_home):
        """Write config to $BMB_ENCOVER_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(bmb_home) / "hindsight"
    ...
    def post_setup(self, bmb_home: str, config: dict) -> None:
        ...
        from bmb_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "bmb")
        ...
        self.save_config(provider_config, bmb_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(bmb_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(bmb_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between BlackMagicBox Encover Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "bmb"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "bmb"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "bmb", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "bmb")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between BlackMagicBox Encover Agent and the User")
