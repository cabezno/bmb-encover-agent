...
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `bmb-{profile}` isolates memory per active Hermes profile. Empty placeholders collapse cleanly (e.g. `bmb-{user}` with no user becomes `bmb`). |
