...
Auto-tracks and cleans up ephemeral files created during BlackMagicBox Encover Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$BMB_ENCOVER_HOME` and `/tmp/hermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `BMB_ENCOVER_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under BMB_ENCOVER_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `BMB_ENCOVER_HOME` or `/tmp/hermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$BMB_ENCOVER_HOME/disk-cleanup/` is itself excluded
- `$BMB_ENCOVER_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
