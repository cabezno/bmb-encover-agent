"""disk_cleanup — ephemeral file cleanup for BlackMagicBox Encover Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under BMB_ENCOVER_HOME)
...
def get_state_dir() -> Path:
    """State dir — separate from ``$BMB_ENCOVER_HOME/logs/``."""
    return get_bmb_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$BMB_ENCOVER_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    ...
    try:
        path.resolve().relative_to(bmb_home)
        return True
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside BMB_ENCOVER_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under BMB_ENCOVER_HOME (but leave BMB_ENCOVER_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "bmb-encover", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(bmb_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == bmb_home:
                continue
            try:
                rel_parts = dirpath.relative_to(bmb_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    bmb_home = get_bmb_home()
    try:
        rel = path.resolve().relative_to(bmb_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "bmb-encover",
        }:
            return None
