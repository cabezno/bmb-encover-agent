...
``bmb_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
bridge in ``bmb_cli/web_server.py``. If you run the dashboard with
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from bmb_cli import web_server as _ws
    except Exception:
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from bmb_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``bmb kanban
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.bmb/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.bmb/profiles/*`` on disk and every
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in bmb_cli/web_server.py.
