...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or BMB exit
...
# display_bmb_home imported lazily at call site (stale-module safety during bmb update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of bmb directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "bmb-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("BMB_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("BMB_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("BMB_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (BMB_INTERACTIVE=1).
    ...
    try:
        os.environ["BMB_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "BMB_SPINNER_PAUSE" in os.environ:
            del os.environ["BMB_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (BMB_INTERACTIVE=1):
    ...
    # interactive BMB password prompt or the sudo -S password-pipe path.
    ...
    # silently without BMB prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and os.getenv("BMB_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so BMB can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, BMBSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.bmb/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Modal credentials/config were found. Log in with `bmb model` or "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(command):
        return (
            ...
            "Use terminal(background=true) so BMB can track the process, then run "
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("BMB_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("BMB_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("BMB_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("BMB_SESSION_USER_ID", "")
                        _gw_user_name = _gse("BMB_SESSION_USER_NAME", "")
            ...
            try:
                from bmb_cli.plugins import invoke_hook
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paid Nous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `bmb model` "
