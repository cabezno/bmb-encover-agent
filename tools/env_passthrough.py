...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a BMB-managed provider credential (API key,
    token, or similar) per ``_BMB_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-BMB API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _BMB_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are BMB-managed provider credentials (from
    ``_BMB_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a BMB-managed
    ...
    Non-BMB third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register BMB provider "
                "credential %r (blocked by _BMB_PROVIDER_ENV_BLOCKLIST). "
