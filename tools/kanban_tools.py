...
1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``bmb kanban complete …``
   inside the container, where ``bmb`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.bmb/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``bmb kanban …``), the dashboard
(``bmb dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``bmb chat`` without the kanban toolset see zero
