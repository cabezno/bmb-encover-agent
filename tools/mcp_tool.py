...
discovers their tools, and registers them into the bmb-encover tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.bmb/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.bmb/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                os.path.join(bmb_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `bmb mcp login {server_name}` (or delete the tokens "
            f"file under ~/.bmb/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.bmb/.env`` loaded at startup).
    """
    try:
