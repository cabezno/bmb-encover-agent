    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "BlackMagicBox Encover Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses BMB_ENCOVER_HOME so each profile gets its own OAuth tokens.
    Layout: ``BMB_ENCOVER_HOME/mcp-tokens/``
    """
    try:
        from bmb_constants import get_bmb_home
        base = Path(get_bmb_home())
    except ImportError:
        base = Path(os.environ.get("BMB_ENCOVER_HOME", str(Path.home() / ".bmb")))
...
# ---------------------------------------------------------------------------
# BMBTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class BMBTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        BMB_ENCOVER_HOME/mcp-tokens/<server_name>.json         -- tokens
        BMB_ENCOVER_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # BMB records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to BMB.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "BlackMagicBox Encover Agent")
    scope = cfg.get("scope")
