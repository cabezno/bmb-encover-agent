...
Configuration is loaded from ~/.bmb/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``bmb_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from bmb_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import bmb_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_bmb_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.bmb/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.bmb/config.yaml.
    ...
    try:
        ...
        logger.debug("bmb_cli.config not available, using default TTS config")
        return {}
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    voice_prompt = str(gemini_config.get("voice_prompt", "")).strip()
    base_url = str(
    ...
    # Prepender voice prompt instructions if configured (for emotion, accent, language control)
    final_text = f"[speak: {voice_prompt}] {text}" if voice_prompt else text
...
def _get_piper_voices_dir() -> Path:
    ...
    """
    from bmb_constants import get_bmb_dir
    root = Path(get_bmb_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.bmb/config.yaml (tts: section).
        ...
        if command_provider_config is not None:
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'bmb-encover[mistral]'"
                }, ensure_ascii=False)
            ...
            try:
                ...
                if "429" in err_str or "quota" in err_str.lower() or "rate limit" in err_str.lower():
                    logger.warning("Gemini TTS quota exceeded, falling back to Edge TTS...")
                    mp3_path = file_str.rsplit(".", 1)[0] + ".mp3"
                    Path(mp3_path).parent.mkdir(parents=True, exist_ok=True)
                    asyncio.run(_generate_edge_tts(text, mp3_path, tts_config))
                    file_str = mp3_path
                    provider = "edge"
                    logger.info("Edge TTS fallback succeeded, file=%s", file_str)
                else:
                    raise
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run bmb setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'bmb setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'bmb tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_bmb_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
