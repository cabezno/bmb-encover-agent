...
Used by bmb_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            bmb_meta = metadata.get("bmb", {})
            if isinstance(bmb_meta, dict):
                tags = bmb_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Encover) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.bmb/skills/ during setup.  They are discoverable via the Skills Hub
