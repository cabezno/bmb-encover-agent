...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `bmb_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `bmb_tools.py` with file-based RPC stubs
...
def generate_bmb_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the bmb_tools.py stub module.
...
def _execute_remote(
    ...
    The script and the generated bmb_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/bmb_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_bmb_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/bmb_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with bmb_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="bmb_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"bmb_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated bmb_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_bmb_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "bmb_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        # the staging tmpdir so ``from bmb_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _bmb_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _bmb_root]
        if _existing_pp:
        ...
        from bmb_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.bmb/.env')).
...
def _load_config() -> dict:
    ...
    ``bmb --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (bmb-encover's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``bmb tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and bmb-encover's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.bmb/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        ...
        f"Available via `from bmb_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into bmb_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from bmb_tools import {import_str}` "
