...
it is automatically downloaded from GitHub releases to $BMB_ENCOVER_HOME/bin/tirith.
...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_bmb_home(), ".tirith-install-failed")
...
def _hermes_bin_dir() -> str:
    """Return $BMB_ENCOVER_HOME/bin, creating it if needed."""
    d = os.path.join(_get_bmb_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $BMB_ENCOVER_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $BMB_ENCOVER_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $BMB_ENCOVER_HOME/bin/tirith
    ...
    bmb_bin = os.path.join(_hermes_bin_dir(), "tirith")
    if os.path.isfile(bmb_bin) and os.access(bmb_bin, os.X_OK):
        _resolved_path = bmb_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return bmb_bin
