...
_FORCE_SYNC_ENV = "BMB_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.bmb") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.bmb to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``BMB_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, bmb_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.bmb/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="bmb-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.bmb/skills/a.md`` →
        ``~/.bmb/skills/a.md``, a new remote file at
        ``/root/.bmb/skills/b.md`` maps to ``~/.bmb/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
