...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from bmb_cli.config import OPTIONAL_ENV_VARS
...
def _make_run_env(env: dict) -> dict:
    ...
    # npm …) into {BMB_ENCOVER_HOME}/home/ when that directory exists.  Only the
...
class LocalEnvironment(BaseEnvironment):
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._bmb_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_bmb_pgid", None)
                    if pgid is None:
                        raise
