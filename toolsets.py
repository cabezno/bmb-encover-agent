...
_BMB_CORE_TOOLS = [
    ...
    "vision_analyze", "image_generate", "design_action", "streaming_action", "brand_action", "director_action", "overlays_director",
    # Skills
    ...
    # spawned as a kanban worker (BMB_KANBAN_TASK env set), otherwise
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (BMB_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full BMB toolsets (CLI + messaging platforms)
    #
    ...
    "bmb-cron": {
        # Mirrors bmb-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `bmb tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as bmb-cli; gated by `bmb tools`",
        "tools": _BMB_CORE_TOOLS,
        "includes": []
    },
    ...
    "bmb-email": {
        "description": "Email bot toolset - interact with BMB via email (IMAP/SMTP)",
        "tools": _BMB_CORE_TOOLS,
        "includes": []
    },
    ...
    "bmb-sms": {
        "description": "SMS bot toolset - interact with BMB via SMS (Twilio)",
        "tools": _BMB_CORE_TOOLS,
        "includes": []
    },
    ...
    "bmb-gateway": {
        ...
        "tools": [],
        "includes": ["bmb-telegram", "bmb-discord", "bmb-whatsapp", "bmb-slack", "bmb-signal", "bmb-bluebubbles", "bmb-homeassistant", "bmb-email", "bmb-sms", "bmb-mattermost", "bmb-matrix", "bmb-dingtalk", "bmb-feishu", "bmb-wecom", "bmb-wecom-callback", "bmb-weixin", "bmb-qqbot", "bmb-webhook", "bmb-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (bmb-<name>).
        # Gives them _BMB_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("bmb-"):
            platform_name = name[len("bmb-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_BMB_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
