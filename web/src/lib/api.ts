...
const SESSION_HEADER = "X-BMB-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__BMB_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the BMB dashboard server");
}
...
export const api = {
  ...
  updateHermes: () =>
    fetchJSON<ActionResponse>("/api/bmb/update", { method: "POST" }),
