...
_loaded_env_paths = load_bmb_dotenv(bmb_home=_bmb_home, project_env=_project_env)
...
from agent.prompt_builder import (
    ...
    BMB_ENCOVER_AGENT_HELP_GUIDANCE,
    KANBAN_GUIDANCE,
)
...
class _SafeWriter:
    ...
    When bmb-encover runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[bmb-encover: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use ~/.bmb/SOUL.md as the primary
        ...
        self._cache_ttl = "5m"
        try:
            from bmb_cli.config import load_config as _load_pc_cfg
        ...
        # both live under ~/.bmb/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from bmb_logging import setup_logging, setup_verbose_logging
        setup_logging(bmb_home=_bmb_home)
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from bmb_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    ...
                    from bmb_cli.models import copilot_default_headers
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        if not _fb_resolved:
                            raise RuntimeError(
                                ...
                                f"variable, or switch to a different provider with `bmb model`."
                            )
                    if not getattr(self, "_fallback_activated", False):
                        # No provider configured — reject with a clear message.
                        raise RuntimeError(
                            "No LLM provider configured. Run `bmb model` to "
                            "select a provider, or run `bmb setup` for first-time "
                            "configuration."
                        )
        ...
        # Session logs go into ~/.bmb/sessions/ alongside gateway sessions
        bmb_home = get_bmb_home()
        self.logs_dir = bmb_home / "sessions"
        ...
        try:
            from bmb_cli.config import load_config as _load_agent_config
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "bmb"
                        except Exception:
                            pass
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from bmb_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from bmb_cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by BlackMagicBox Encover Agent.  Choose a model with at least "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                session_id=self.session_id,
                source=self.platform or os.environ.get("BMB_SESSION_SOURCE", "cli"),
                model=self.model,
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least BMB's minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from bmb_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        turn-scoped).
        """
        from bmb_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from bmb_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``bmb chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `bmb setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by BMB "
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from bmb_cli.models import _should_use_copilot_responses_api
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("BMB_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("BMB_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Some execution modes (cron) still want BMB_ENCOVER_HOME persona while keeping
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the bmb-encover install
        ...
        from bmb_time import now as _bmb_now
        now = _bmb_now()
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from bmb_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from bmb_cli.copilot_auth import resolve_copilot_token
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            try:
                from bmb_cli.model_normalize import normalize_model_for_provider
    ...
    def _try_recover_primary_transport(
        ...
        Skipped for proxy/aggregator providers (OpenRouter, etc) which
        ...
        def _shrink_data_url(url: str) -> Optional[str]:
            ...
            try:
                ...
                tmp = tempfile.NamedTemporaryFile(
                    prefix="bmb_shrink_", suffix=suffix, delete=False,
                )
                try:
                    tmp.write(raw)
                    tmp.close()
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        _is_nous = "blackmagicbox" in self._base_url_lower
        ...
        # Anthropic max output for Claude on OpenRouter
        _ant_max = None
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "bmb",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Portal.
        """
        if base_url_host_matches(self._base_url_lower, "blackmagicbox.com"):
            return True
        ...
        if (
            ...
            try:
                from bmb_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from bmb_cli.models import lmstudio_model_reasoning_options
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        if not pre_tool_block_checked:
            try:
                from bmb_cli.plugins import get_pre_tool_call_block_message
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=bmb-encover"]
    ...
    def run_conversation(
        ...
        # ``bmb logs --session <id>`` can filter a single conversation.
        from bmb_logging import set_session_context
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Legacy cleanup: bmb config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: bmb config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `bmb auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: bmb setup", force=True)
